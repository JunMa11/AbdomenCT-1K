...
if __name__ == "__main__":
    ...
    base_label_nih = "../data/RESULTS_FOLDER/nnUNet/3d_fullres/Task200_SpleenOrgan/nnUNetTrainerV2__nnUNetPlansv2.1/all/predictions_NIH/"
    base_label_msd = "../data/RESULTS_FOLDER/nnUNet/3d_fullres/Task200_SpleenOrgan/nnUNetTrainerV2__nnUNetPlansv2.1/all/predictions_MSD/"
    out = "../data/nnUNet_raw_data/Task214_SemiNIHMSDSplBase"
    cases = list(np.sort(os.listdir(base_label_nih)))
    cases += list(np.sort(os.listdir(base_label_msd)))
    cases = [x for x in cases if x.endswith('nii.gz')]
    ...
    for c in cases:
        case_id = c.split("_")[0]
        if case_id == "pancreas":
            if int(c.split(".")[0].split("_")[1]) > 421:
                shutil.copy(join(base_image, "NIHPanTr82", c[:-7] + "_0000.nii.gz"), join(out, "imagesTr", c[:-7] + "_0000.nii.gz"))
                shutil.copy(join(base_label_nih, c), join(out, "labelsTr", c))
                ...
                shutil.copy(join(base_label_msd, c), join(out, "labelsTr", c))
                print(join(out, "imagesTr", c[:-7] + "_0000.nii.gz"))
    
    json_dict = {}
    json_dict['name'] = "SpleenOrgan"
