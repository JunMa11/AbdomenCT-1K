...
if __name__ == "__main__":
    base_image = "/dfsdata2/zhangyao_data/Tasks/Abdomen1K/nnUNet/nnunet/data/nnUNet_raw_data/Task401_ContinualUp/imagesTr"
    base_label = "/dfsdata2/zhangyao_data/Tasks/Abdomen1K/nnUNet/nnunet/data/nnUNet_raw_data/Task401_ContinualUp/labelsTr"
    out = "../data/nnUNet_raw_data/Task400_ContinualBase"
    ...
    base_liver = "/dfsdata2/zhangyao_data/Tasks/Abdomen1K/nnUNet/nnunet/data/RESULTS_FOLDER/nnUNet/3d_fullres/Task440_MSDLiverBase/nnUNetTrainerV2__nnUNetPlansv2.1"
    base_kidney = "/dfsdata2/zhangyao_data/Tasks/Abdomen1K/nnUNet/nnunet/data/RESULTS_FOLDER/nnUNet/3d_fullres/Task420_KiTSBase/nnUNetTrainerV2__nnUNetPlansv2.1"
    base_spleen = "/dfsdata2/zhangyao_data/Tasks/Abdomen1K/nnUNet/nnunet/data/RESULTS_FOLDER/nnUNet/3d_fullres/Task410_SpleenBase/nnUNetTrainerV2__nnUNetPlansv2.1"
    base_pancreas = "/dfsdata2/zhangyao_data/Tasks/Abdomen1K/nnUNet/nnunet/data/RESULTS_FOLDER/nnUNet/3d_fullres/Task430_MSDPanBase/nnUNetTrainerV2__nnUNetPlansv2.1"
    
    for c in cases:
        nii_volume = nib.load(join(base_image, c[:-7] + "_0000.nii.gz"))
        seg_volume = np.zeros(shape=nii_volume.get_fdata().shape, dtype=np.uint8)
        affine = nii_volume.affine

        if os.path.isfile(join(base_liver, "gt_niftis", c)):
            liver_seg_volume = nib.load(join(base_liver, "gt_niftis", c)).get_fdata()
            kidney_seg_volume = nib.load(join(base_kidney, "all/predictions", c)).get_fdata()
            spleen_seg_volume = nib.load(join(base_spleen, "all/predictions", c)).get_fdata()
            pancreas_seg_volume = nib.load(join(base_pancreas, "all/predictions", c)).get_fdata()    
        elif os.path.isfile(join(base_kidney, "gt_niftis", c)):
            liver_seg_volume = nib.load(join(base_liver, "all/predictions", c)).get_fdata()
            kidney_seg_volume = nib.load(join(base_kidney, "gt_niftis", c)).get_fdata()
            spleen_seg_volume = nib.load(join(base_spleen, "all/predictions", c)).get_fdata()
            pancreas_seg_volume = nib.load(join(base_pancreas, "all/predictions", c)).get_fdata()
        elif os.path.isfile(join(base_spleen, "gt_niftis", c)):
            liver_seg_volume = nib.load(join(base_liver, "all/predictions", c)).get_fdata()
            kidney_seg_volume = nib.load(join(base_kidney, "all/predictions", c)).get_fdata()
            spleen_seg_volume = nib.load(join(base_spleen, "gt_niftis", c)).get_fdata()
            pancreas_seg_volume = nib.load(join(base_pancreas, "all/predictions", c)).get_fdata()
        elif os.path.isfile(join(base_pancreas, "gt_niftis", c)):
            liver_seg_volume = nib.load(join(base_liver, "all/predictions", c)).get_fdata()
            kidney_seg_volume = nib.load(join(base_kidney, "all/predictions", c)).get_fdata()
            spleen_seg_volume = nib.load(join(base_spleen, "all/predictions", c)).get_fdata()
            pancreas_seg_volume = nib.load(join(base_pancreas, "gt_niftis", c)).get_fdata()
        else:
            raise Exception('hahaha')
            
        shutil.copy(join(base_image, c[:-7] + "_0000.nii.gz"), join(out, "imagesTr", c[:-7] + "_0000.nii.gz"))
        seg_volume[liver_seg_volume == 1] = 1
        seg_volume[kidney_seg_volume == 1] = 2
        seg_volume[spleen_seg_volume == 1] = 3
        seg_volume[pancreas_seg_volume == 1] = 4
        nib.save(nib.Nifti1Image(seg_volume, affine), join(out, "labelsTr", c))
        print(join(out, "imagesTr", c[:-7] + "_0000.nii.gz"))

    json_dict = {}
    json_dict['name'] = "ContinualUp"
    ...
    json_dict['numTest'] = 0
    json_dict['training'] = [{'image': "./imagesTr/%s" % i, "label": "./labelsTr/%s" % i} for i in cases]
    json_dict['test'] = []
