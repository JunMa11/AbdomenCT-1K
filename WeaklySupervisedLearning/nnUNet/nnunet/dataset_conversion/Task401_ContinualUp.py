...
if __name__ == "__main__":
    ...
    out = "../data/nnUNet_raw_data/Task401_ContinualUp"
    
    cases = np.sort(os.listdir(base_label))
    cases = [x for x in cases if (x.split("_")[0] == "pancreas" and int(x.split(".")[0].split("_")[1]) <= 421) or x.split("_")[0] == "case" or x.split("_")[0] == "spleen"]
    
    base_test_image = "/dfsdata2/zhangyao_data/DB/Abdomen1K/Task100_FourAbdOrgans/imagesTs"
    base_test_label = "/dfsdata2/zhangyao_data/DB/Abdomen1K/Task100_FourAbdOrgans/labelsTs"
    test_cases = np.sort(os.listdir(base_test_label))
    test_cases = [x for x in test_cases if x.split("_")[0] == "pancreas" and int(x.split(".")[0].split("_")[1]) <= 421]
    ...
    for c in cases:
        case_id = c.split("_")[0]
        if case_id == "pancreas":
            shutil.copy(join(base_image, "MSDPan281", c[:-7] + "_0000.nii.gz"), join(out, "imagesTr", c[:-7] + "_0000.nii.gz"))
            ...
            shutil.copy(join(base_image, "KITSTr210", c[:-7] + "_0000.nii.gz"), join(out, "imagesTr", c[:-7] + "_0000.nii.gz"))
            shutil.copy(join(base_label, c), join(out, "labelsTr", c))
            print(join(out, "imagesTr", c[:-7] + "_0000.nii.gz"))
        
    for c in test_cases:
        shutil.copy(join(base_test_image, c[:-7] + "_0000.nii.gz"), join(out, "imagesTr", c[:-7] + "_0000.nii.gz"))
        shutil.copy(join(base_test_label, c), join(out, "labelsTr", c))
        print(join(out, "imagesTr", c[:-7] + "_0000.nii.gz"))
    ...
    json_dict['numTest'] = 0
    json_dict['training'] = [{'image': "./imagesTr/%s" % i, "label": "./labelsTr/%s" % i} for i in (cases + test_cases)]
    json_dict['test'] = []
