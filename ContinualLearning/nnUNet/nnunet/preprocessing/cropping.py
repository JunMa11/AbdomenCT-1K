...
def get_bbox_from_mask(mask, outside_value=0):
    ...
    maxzidx = int(np.min([np.max(mask_voxel_coords[0])+1, mask.shape[0]]))
    minxidx = int(np.min(mask_voxel_coords[1]))
    maxxidx = int(np.min([np.max(mask_voxel_coords[1])+1, mask.shape[1]]))
    minyidx = int(np.min(mask_voxel_coords[2]))
    maxyidx = int(np.min([np.max(mask_voxel_coords[2])+1, mask.shape[2]]))
