...
def verify_same_geometry(img_1: sitk.Image, img_2: sitk.Image):
    ori1, spacing1, direction1, size1 = np.round(img_1.GetOrigin()), np.round(img_1.GetSpacing()), np.round(img_1.GetDirection()), img_1.GetSize()
    ori2, spacing2, direction2, size2 = np.round(img_2.GetOrigin()), np.round(img_2.GetSpacing()), np.round(img_2.GetDirection()), img_2.GetSize()
    ...
    if same_spac and same_dir and same_size:
        return True
    else:
        return False
