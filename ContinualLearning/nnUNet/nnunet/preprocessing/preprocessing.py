...
class GenericPreprocessor(object):
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        ...
        for c in range(len(data)):
            ...
            print('Normalization_scheme_per_modality: ', scheme)
            if scheme == "CT":
                ...
                # input CT scans have intensities [0,255], just normalized by z-score
                print('*'*30)
                print('Input intensity 0, 255, normalized by zscore!')
                data[c] = (data[c] - np.mean(data[c])) / np.std(data[c])
                if use_nonzero_mask[c]:
                    data[c][seg[-1] < 0] = 0            
            elif scheme == "CT2":
                ...
                print('3D PNG; normalized by divided 255')
                data[c] = data[c] / np.max(data[c])
            else:
                print('Normalized just by z-score!')
                if use_nonzero_mask[c]:
                    mask = seg[-1] >= 0
                else:
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        ...
        for c in range(len(data)):
            ...
            if scheme == "CT":
                ...
                print('2D PNG; normalized by divided 255')
                data[c] = data[c] / np.max(data[c])
            else:
                if use_nonzero_mask[c]:
                    mask = seg[-1] >= 0
