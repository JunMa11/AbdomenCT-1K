...
if __name__ == "__main__":
    out = "../../data/nnUNet_raw_data/Task232_SemiSplPanLiTSKiTS"
    ...
    labelled_image = "/dfsdata2/zhangyao_data/DB/Abdomen1K/Semi/imagesTr/"
    labelled_label = "/dfsdata2/zhangyao_data/DB/Abdomen1K/Semi/labelsTr/"
    
    unlabelled_image = "/dfsdata2/zhangyao_data/DB/Abdomen1K/Semi/unlabelled_imagesTr/"
    unlabelled_label = "/dfsdata2/zhangyao_data/DB/Abdomen1K/Semi/pseudo_labelsTr/"
    
    labelled_cases = np.sort(glob.glob(labelled_image+"*.nii.gz"))
    unlabelled_cases = np.sort(glob.glob(unlabelled_image+"*.nii.gz"))
    cases = list(labelled_cases) + list(unlabelled_cases)
    cases = [x.replace('_0000.nii.gz', '.nii.gz') for x in cases]

    for c in labelled_cases:
        case_filename = c.split('/')[-1]
        shutil.copy(c, join(out, "imagesTr", case_filename))
        shutil.copy(join(labelled_label, case_filename.replace('_0000.nii.gz', '.nii.gz')), join(out, "labelsTr", case_filename.replace('_0000.nii.gz', '.nii.gz')))
        print(join(out, "imagesTr", case_filename))
        
    for c in unlabelled_cases:
        case_filename = c.split('/')[-1]
        shutil.copy(c, join(out, "imagesTr", case_filename))
        shutil.copy(join(unlabelled_label, case_filename.replace('_0000.nii.gz', '.nii.gz')), join(out, "labelsTr", case_filename.replace('_0000.nii.gz', '.nii.gz')))
        print(join(out, "imagesTr", case_filename))

    json_dict = {}
    json_dict['name'] = "SemiSplPanLiTSKiTS"
